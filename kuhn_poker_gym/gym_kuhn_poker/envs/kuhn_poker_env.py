...
from .one_hot_space import OOneHotEncoding
...
class KuhnPokerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # print(f"Player {self.current_player} has {move.name}")
    ...
    def get_winner(self):
        ...
        # print(f"Eligible players: {self.elegible_players}")
    ...
    def random_initial_state_vector(self):
        ...
        # print('Dealt cards:', dealt_cards_per_player)
    ...
    def vector_from_dealt_hands(self, dealt_cards_per_player):
        ...
        for dealt_card in dealt_cards_per_player:
            ...
            hand[dealt_card] = 1
            player_hands += hand
        # print(f"Dealt hands: {player_hands}")
        return player_hands
    ...
    def calculate_observation_space(self):
        player_id  = OOneHotEncoding(self.number_of_players)
        dealt_card = OOneHotEncoding(self.deck_size)

        betting_states = [OOneHotEncoding(len(ActionType) + 1)
                          for _ in range(self.number_of_players)
                          for _ in range(self.betting_rounds)]
