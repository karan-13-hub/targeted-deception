...
def get_config():
    ...
    parser.add_argument('--lr_decay_gamma', type=float, default=0.99, help='Learning rate decay gamma')
...
def main():
    ...
    # Create a matrix-style table with models as rows and columns
    if config.all_against_all:
        model_types = list(agents.keys())
        matrix_data = {}
        ...
        for model1 in model_types:
            ...
            for model2 in model_types:
                matrix_data[model1][model2] = None
        
        # Fill matrix with win rates
        for match, data in results.items():
            model_pair = match.split(" vs ")
            if len(model_pair) == 2:
                model1, model2 = model_pair
                
                # Check if this is a self-play match (same model vs itself)
                if model1 == model2:
                    # For self-play on diagonal, take the first player's win rate
                    for key, value in data.items():
                        if "Player 0 Win Rate" in key:
                            matrix_data[model1][model2] = f"{value:.3f}"
                else:
                    # Store the win rate of model1 against model2
                    # This means row vs column
                    for key, value in data.items():
                        if model1 in key and "Win Rate" in key:
                            matrix_data[model1][model2] = f"{value:.3f}"
        
        # Create DataFrame from matrix data
        matrix_df = pd.DataFrame(matrix_data).transpose()  # Transpose to make rows vs columns correct
        print("Win rates matrix (row model vs column model):")
        print(matrix_df)
    else:
